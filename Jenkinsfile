            ...
            steps {
                dir('.') {
                    bat """
                        ...
                        echo Using RegTest jar: ${REGTEST}
                        
                        echo Checking if regtest jar exists...
                        if not exist "${REGTEST}" (
                            echo ERROR: RegTest jar not found at ${REGTEST}
                            exit /b 1
                        )
                        ...
                        java -jar "${REGTEST}" -project . -host ${BRIDGE_HOST} -port ${BRIDGE_PORT} -username ${BRIDGE_USER} -password ${BRIDGE_PASSWORD} -list
                        
                        echo.
                        echo Running all available regression tests...
                        java -jar "${REGTEST}" -project . -host ${BRIDGE_HOST} -port ${BRIDGE_PORT} -username ${BRIDGE_USER} -password ${BRIDGE_PASSWORD} -logfile regressiontest/result.xml
